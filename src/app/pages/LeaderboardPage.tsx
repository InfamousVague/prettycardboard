...
import { Avatar, Heading, Pill, SegmentedControl, Size, Text, TextTone } from '@glacier/react';
import { useT } from '../i18n.ts';
import { useApp } from '../state/appStore.ts';
import * as api from '../net/api.ts';
import type { LadderEntry } from '../net/types.ts';
...
export function LeaderboardPage() {
  ...
  const mine = entries?.find((e) => e.userId === identity?.userId) ?? null;
    ...
    <div className="lbPage">
      ...
      {mine ? (
        <section className="lbMine">
          <span aria-hidden>
            <RankBadge division={divisionFor(mine.rating)} size={44} />
          </span>
          <div className="lbMineWho">
            <span className="lbMineLabel">{divisionFor(mine.rating).label}</span>
            <span className="lbMineSub">
              {t('lbYouAre')} #{mine.position} · {mine.rating} · {mine.wins}W {mine.losses}L
            </span>
          </div>
        </section>
      ) : entries && identity ? (

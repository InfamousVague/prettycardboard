...
- `GET /api/rooms/mine` → rooms where the caller holds a seat:
  `[{roomId, code, name, seats, host, persistent, started, updatedAt,
     players: [{userId, username, online}]}]`, newest activity first.
  Membership in `players` is the only criterion, so a seat you never give up
  keeps its table on this list. `host` is the room's owner — do not infer it
  from `players[0]`, which is seat order.
...
- `DELETE /api/rooms/{id}/seat` → 204. Gives up the CALLER's own seat, from
  anywhere — no host rights and no socket in the room required. This is the
  only self-service way out of `players` on a persistent table: the WS
  `room.leave` treats such a table as saved and merely marks the seat offline.
  Removes the caller from `players`/`spectators`, drops their stack and
  pending-commander entries, hands `host` to the next human if the caller held
  it, and deletes the room outright once no human seats remain. Everyone still
  seated gets a fresh room state; the leaver does not.
  `404 not_found` if there is no such room or the caller has no seat at it.
  `409 in_progress` once the game has started and has no result — settle it by
  conceding at the table, so the loss and the ratings still land.
...
- `{ "type": "bot.add", "deckCode"?: "FIC-<n>"|"random", "style"?:
  "casual"|"aggro"|"defensive" }` — host only, room not started, Magic,
  Yu-Gi-Oh and Mood Swings tables only, at least one free seat. Seats a
  synthetic player (`userId` `"bot:<id>"`,
...
### Mood Swings bots (2026-08-04)

`bot.add` answered `bad_game` at a Mood Swings table, so the one game in the
app that needs no deckbuilding and takes five minutes had no way to be played
alone. Bots now sit down there too, with a third brain
(`server/src/bot/moodswings.rs`) - Mood Swings shares even less with Magic
than a duel does: no mana, no combat, no permanents, and a turn that is one
card and a number.

- **Boxes**: `server/src/data/bot_decks_moodswings.json`, 8 boxes, generated by
  `scripts/gen-mood-decks.mjs` from `src/app/data/moodswingsCards.json`. Each
  is the product itself - 45 distinct cards off the 133-card set - rolled from
  a fixed seed so the pool is stable across reruns. There are more boxes than
  the game has seats so four bots never collide. Rerun after
  `scripts/sync-moodswings.mjs`. The real game deals one SHARED pile; the
  freeform engine has no shared zone, so every seat brings a box of its own,
  which is already what a human seat does here.
- **What the bot does**: sweeps last round's mood into the discard at the start
  of its turn (its turn coming round again *is* the new round - a round is one
  turn each), plays exactly one mood face up in a readable row, sets its Score
  counter to what its side is printing, and passes. Style picks the mood:
  aggro plays its highest, defensive the cheapest one that still beats the
  best total on the table (so it still has an [8] in round three), casual the
  middle of its hand.
- **What it does NOT do**: read card text, or judge the round. There is no
  Mood Swings oracle, so a conditional second value is never resolved - the
  bot plays the PRINTED value, the same read the client takes. And rounds stay
  a manual counter: judging is a table-wide comparison, and a bot that both
  judged and cleaned up would race the others, since the first seat to sweep
  would leave every later seat comparing against a board that had gone.
- **No untap step on a Mood Swings table**, for the same reason a duel has
  none: sideways means SUPPRESSED (the mood counts as [0]), which is a state
  and not a spent resource. `auto_turn_begin` would otherwise stand a
  suppressed mood back up to its printed value mid-round.

`quickplay` deals from the box pool too, and the lobby's empty-seat menu and
the developer Bots tab both offer Mood Swings.

### Yu-Gi-Oh bots (2026-08-01)

`bot.add` used to answer `bad_game` at anything but a Magic table, so the
duel game had no practice opponent. Bots now play Yu-Gi-Oh too, with their
own brain (`server/src/bot/yugioh.rs`) - none of the Magic machinery applies,
since a duel has no mana, no stack and no blocks.

- **Decks**: `server/src/data/bot_decks_yugioh.json`, 52 decks (the two
  bundled starter products plus 50 archetype builds), generated by
  `scripts/gen-yugioh-decks.mjs` from the cached YGOPRODeck catalog
  (`public/cache/yugioh/catalog.json` - no network I/O; run `sync-yugioh`
  first). Each is a 40-card Main Deck (20 monsters / 12 spells / 8 traps) with
  its Extra Deck on the `commander` board. Rerun after a catalog sync.
- **What the bot does**: one Normal Summon per turn, paying tributes for
  Level 5+; Sets a monster in Defense Position when it would be outclassed on
  arrival; Sets its backrow (Field Spells to the Field Zone); declares
  attacks and settles battle damage. Cards land in the printed zone lattice,
  using the same coordinates as `src/app/pages/table/yugiohZones.tsx`.
- **What it does NOT do**: activate card effects. There is no duel oracle, so
  the bot never pretends to resolve text it cannot read - the same closed-set
  rule the Magic oracle follows.

Three engine changes came with it, and they apply to human duels too:

- `combat.attack` gains optional **`targetIid`**: a Yu-Gi-Oh attack
  declaration names the monster it battles, where Magic declares at a player
  and answers with blocks. It rides `Attacker.targetIid` in `RoomState`, and
  the log line reads "X attacks Y" (a face-down target is announced as "a set
  card"). Magic leaves it unset; nothing else changes.
- **No untap step on a duel table.** A rotated card is in Defense Position,
  not tapped, so `auto_turn_begin` no longer straightens the board - it was
  standing every Set monster up into Attack Position each turn.
- **Declaring an attack does not rotate the attacker** at a duel table, for
  the same reason: nothing taps to attack, and rotating would put the
  attacker in the one position that cannot.

### Bot decks by format (2026-08-01)

Bots drew from four Commander precons and deck choice ignored the room, so a
bot at a STANDARD table brought a 100-card Commander deck. Two changes:

- `server/src/data/bot_decks_standard.json` ships 50 Standard-legal 60-card
  decks, generated by `scripts/gen-standard-decks.mjs` from Scryfall'''s
  public search API using archetype recipes (aggro / midrange / control /
  ramp / tempo over five monocolours and ten guilds). The script caches
  responses in `scripts/.scryfall-cache.json` and backs off on 429; DELETE
  that cache and rerun after each Standard rotation. Basic-land mana bases
  only - a bot that stumbles on colours is a worse opponent than one with a
  clean base.
- `bot.add` picks from the pool matching `room.format`: Standard tables get
  the Standard pool, everything else keeps the precons. An explicit
  `deckCode` still overrides, and an unknown code is still refused.

Bot deck entries carry an optional `format` tag; the precons predate it and
default to none (= not Standard).

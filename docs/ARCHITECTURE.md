...
`BrowseCatalog` is a **single layout both games share**. It renders a toolbar (search, facet chips, sort segmented control, optional group-by control), a featured shelf, and grids grouped by the active group-mode; each tile's "Add" calls `api.createDeck` and jumps to `/decks`. It knows nothing game-specific — it operates entirely on the normalized `BrowseDeck` shape (name, cover, art, badge, `identity` node, `facets[]`, `groups{}`, `sortDate`, `cards`, `game`, `format`). `BrowsePage` is the adapter layer: a `GameSelect` picker (a glacier `MultiSelect`, persisted comma-joined in `sessionStorage['pc_browse_game']`) chooses **one or more** catalogs, then a `useMemo` per game maps `CATALOG` (MTG precons, grouped by year/set, faceted by WUBRG), `cyberpunkCatalog()` (per-Legend decks, grouped/faceted by color), the Yu-Gi-Oh deck products, and the two Mood Swings products (a randomized 45-card box and the complete 133-card set) into `BrowseDeck[]`. Only the picked games contribute decks, facet rows, and group modes — a game with no catalog contributes nothing rather than falling through to Magic's — and a mixed selection adds a "by game" group mode. Clearing the picker means every game.

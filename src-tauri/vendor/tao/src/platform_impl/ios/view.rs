...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // PATCH (not upstream 0.35.3): as_ptr only BORROWS, so `config` was
      // dropped and freed the moment this function returned - UIKit then
      // objc_retain'd freed memory and the app segfaulted in
      // _connectUISceneFromFBSScene before drawing a frame. This is upstream's
      // own fix, landed verbatim in tao 0.36.0, which we cannot take because
      // tauri-runtime-wry 2.11.4 pins tao ^0.35.

...
pub fn deck_meta(code: &str) -> Option<serde_json::Value> {
    ...
    if let Some(deck) = mood_data().decks.iter().find(|deck| deck.code == code) {
        return Some(mood_deck_meta(deck));
    }
    let d = data();
...
/// The Mood Swings pool, generated by scripts/gen-mood-decks.mjs. A "deck" here
/// is a BOX: 45 of the set's 133 cards, singleton, rolled once and embedded.
#[derive(Deserialize)]
pub struct MoodData {
    pub decks: Vec<BotDeck>,
    pub attrs: HashMap<String, MoodAttr>,
}

/// One mood as the bot reads it. There is nothing else on the card it can act
/// on: the rules text is conditional prose with no oracle behind it, and the
/// frame colour has no rules meaning of its own - so the printed value is the
/// whole of the bot's knowledge.
#[derive(Deserialize, Clone)]
pub struct MoodAttr {
    /// The PRINTED value - the first of the card's values. A third of the set
    /// swaps to a second value under a condition the server cannot evaluate.
    pub v: i64,
}

static MOOD: OnceLock<MoodData> = OnceLock::new();

pub fn mood_data() -> &'static MoodData {
    MOOD.get_or_init(|| {
        serde_json::from_str(include_str!("../data/bot_decks_moodswings.json"))
            .expect("bot_decks_moodswings.json parses")
    })
}

pub(crate) fn mood_attr(card: &Card) -> Option<&'static MoodAttr> {
    card.scryfall_id.as_deref().and_then(|sid| mood_data().attrs.get(sid))
}

/// What a mood is worth where it now sits. Sideways means SUPPRESSED in this
/// game, and a suppressed mood counts as [0] - the one state the server can
/// read off the card without an oracle.
pub(crate) fn mood_value(card: &Card) -> i64 {
    if card.tapped {
        return 0;
    }
...
/// A box's public metrics. Mood Swings has no card types to count and no curve
/// to draw, so this is the honest minimum plus the one number the game is about:
/// the printed value sitting in the box.
fn mood_deck_meta(deck: &BotDeck) -> serde_json::Value {
    let attrs = &mood_data().attrs;
    let size: u64 = deck.cards.iter().map(|c| c.qty as u64).sum();
    let value: i64 =
        deck.cards.iter().filter_map(|c| attrs.get(&c.sid).map(|a| a.v * c.qty as i64)).sum();
    serde_json::json!({
        "size": size,
        "cover": deck.cards.first().map(|c| c.sid.clone()),
        "totalValue": value,
    })
}
...
/// pool, a Mood Swings table a box off its own set; a Standard table the
/// 60-card Standard pool; everything else keeps the Commander precons (the
/// historical behaviour, and the right answer for Commander, Brawl and freeform
/// pods alike). Falls back to the whole pool if a format somehow has no decks,
/// so a bot always brings SOMETHING.
pub fn decks_for(game: &str, format: &str) -> Vec<&'static BotDeck> {
    if game == "yugioh" {
        return ygo_data().decks.iter().collect();
    }
    if game == "moodswings" {
        return mood_data().decks.iter().collect();
    }
    let all = &data().decks;
